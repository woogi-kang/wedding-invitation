...
// Generate petals - reduced count for mobile
...
export function IntroPage({ onEnter }: IntroPageProps) {
  ...
  // Detect mobile and generate appropriate petal count
  useEffect(() => {
    const mobile = window.innerWidth < 768;
    setIsMobile(mobile);
    // Mobile: 25 petals, Desktop: 50 petals (reduced from 70)
    setPetals(generatePetals(mobile ? 25 : 50));
  }, []);

  // Initialize Vara.js for handwriting animation
  useEffect(() => {
    if (!varaContainerRef.current || varaInstanceRef.current) return;
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center overflow-hidden">
      ...
      {/* Central glow - blur disabled on mobile */}
      <motion.div
