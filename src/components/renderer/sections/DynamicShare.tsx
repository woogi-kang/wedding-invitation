...
import type { DynamicShareProps } from '../types';
...
export function DynamicShare({ groomName, brideName, date, dateDisplay, venue }: DynamicShareProps) {
  ...
  return (
    <Section id="share" background="secondary">
      <div ref={sectionRef} className="max-w-md mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8, delay: 0.1 }} className="mb-6">
          <button
          ...
          >
            <div className="flex h-10 w-10 items-center justify-center rounded-full" style={{ backgroundColor: 'var(--color-botanical-light)' }}>
              <Calendar className="h-5 w-5" style={{ color: 'var(--color-botanical)' }} />
            </div>
            <div className="text-left">
              <p className="text-sm font-medium" style={{ fontFamily: 'var(--font-heading)', color: 'var(--color-text)' }}>캘린더에 저장</p>
              <p className="text-xs" style={{ color: 'var(--color-text-muted)' }}>결혼식 일정을 캘린더에 추가합니다</p>
            </div>
          </button>
        </motion.div>

        <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8, delay: 0.2 }} className="flex justify-center gap-6">
          <button onClick={handleKakaoShare} className="flex flex-col items-center gap-2 transition-opacity hover:opacity-70">
            <div className="flex h-12 w-12 items-center justify-center rounded-full" style={{ backgroundColor: '#FEE500' }}>
              <KakaoIcon className="h-6 w-6 text-[#3C1E1E]" />
            </div>
            <span className="text-xs" style={{ color: 'var(--color-text-light)' }}>카카오톡</span>
          </button>
          <button onClick={handleCopyLink} className="flex flex-col items-center gap-2 transition-opacity hover:opacity-70">
            ...
            <span className="text-xs" style={{ color: 'var(--color-text-light)' }}>링크 복사</span>
          </button>
          <button onClick={handleShare} className="flex flex-col items-center gap-2 transition-opacity hover:opacity-70">
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-white border border-[var(--color-border)]">
              <Share2 className="h-5 w-5" style={{ color: 'var(--color-primary)' }} />
            </div>
            <span className="text-xs" style={{ color: 'var(--color-text-light)' }}>공유</span>
          </button>
        </motion.div>
      </div>
    </Section>
