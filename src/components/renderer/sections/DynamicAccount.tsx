...
import type { DynamicAccountProps, AccountInfo } from '../types';
...
function AccountGroup({ title, accounts, isOpen, onToggle, accentColor }: AccountGroupProps) {
  ...
  return (
    <div className="border-b border-[var(--color-border-light)] last:border-b-0">
      ...
      >
        <span className="text-base" style={{ fontFamily: 'var(--font-heading)', color: accentColor }}>
          {title}
        </span>
          ...
          >
            <div className="px-4 pb-5 space-y-3">
              {validAccounts.map((account, index) => (
                <div key={index} className="flex items-center justify-between p-4 rounded-sm" style={{ backgroundColor: 'var(--color-secondary)' }}>
                  <div>
                    <p className="text-xs mb-1" style={{ color: 'var(--color-text-muted)' }}>{account.bank}</p>
                    <p className="text-sm tracking-wider mb-1" style={{ fontFamily: 'var(--font-heading)', color: 'var(--color-text)' }}>{account.number}</p>
                    <p className="text-sm" style={{ color: 'var(--color-text-light)' }}>{account.holder}</p>
                  </div>
                  <button
...
export function DynamicAccount({ groomAccounts, brideAccounts }: DynamicAccountProps) {
    ...
    <Section id="account" background="white">
      <div ref={sectionRef} className="max-w-md mx-auto">
        ...
        >
          ...
          <p className="text-sm leading-relaxed" style={{ fontFamily: 'var(--font-body)', color: 'var(--color-text-light)' }}>
            함께하지 못하시더라도<br />축하의 마음을 전해주실 분들을 위해<br />계좌번호를 안내드립니다.<br /><br />보내주시는 마음, 감사히 간직하겠습니다.
          </p>
        </motion.div>
