...
import type { DynamicCoupleIntroProps } from '../types';

export function DynamicCoupleIntro({ groom, bride }: DynamicCoupleIntroProps) {
        ...
        <div className="space-y-5 flex flex-col items-center">
          ...
          >
            {groom.photo && (
              <div
                ...
                style={{ backgroundImage: `url(${groom.photo})`, border: '3px solid var(--color-groom)' }}
              />
            )}
            <div className="flex-1">
              <p
                className="text-[10px] tracking-[0.3em] uppercase mb-1"
                style={{ fontFamily: 'var(--font-accent)', color: 'var(--color-groom)', position: 'relative', top: '-3px' }}
              >
                Groom
              </p>
              <p className="text-xl mb-2" style={{ fontFamily: 'var(--font-heading)', color: 'var(--color-text)' }}>
                {groom.name}
              </p>
              <p className="text-xs" style={{ fontFamily: 'var(--font-body)', color: 'var(--color-text-light)' }}>
          ...
          >
            {bride.photo && (
              <div
                className="w-24 h-24 rounded-full bg-cover bg-center flex-shrink-0"
                style={{ backgroundImage: `url(${bride.photo})`, border: '3px solid var(--color-bride)' }}
              />
            )}
            <div className="flex-1 text-right">
              <p
                className="text-[10px] tracking-[0.3em] uppercase mb-1"
                style={{ fontFamily: 'var(--font-accent)', color: 'var(--color-bride)', position: 'relative', top: '-3px' }}
              >
                Bride
              </p>
              <p className="text-xl mb-2" style={{ fontFamily: 'var(--font-heading)', color: 'var(--color-text)' }}>
