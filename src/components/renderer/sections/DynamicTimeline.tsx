...
import type { DynamicTimelineProps } from '../types';
...
export function DynamicTimeline({ milestones }: DynamicTimelineProps) {
    ...
    <Section id="timeline" background="secondary">
      <div ref={sectionRef} className="max-w-lg mx-auto">
        ...
        <div className="relative">
          <div className="absolute left-1/2 top-0 bottom-0 w-px -translate-x-1/2" style={{ backgroundColor: 'var(--color-border)' }} />
              ...
              return (
                ...
                >
                  <div className={`w-[calc(50%-24px)] ${isLeft ? 'text-right pr-4' : 'text-left pl-4'}`}>
                    <p className="text-xs mb-1" style={{ fontFamily: 'var(--font-accent)', color: 'var(--color-primary)' }}>{item.date}</p>
                    <p className="text-sm font-medium mb-1" style={{ fontFamily: 'var(--font-heading)', color: 'var(--color-text)' }}>{item.title}</p>
                    {item.description && (
                      <p className="text-xs" style={{ fontFamily: 'var(--font-body)', color: 'var(--color-text-light)' }}>{item.description}</p>
                    )}
                  </div>
                  <div
                    className="absolute left-1/2 -translate-x-1/2 w-12 h-12 rounded-full flex items-center justify-center z-10"
                    style={{ backgroundColor: 'var(--color-white)', border: '2px solid var(--color-primary)' }}
                  >
